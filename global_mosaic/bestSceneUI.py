...
class ThumbHistApp():
    def __init__(self):
        self.wd = tkinter.Tk()
        self.wd.title(u'Landsat数据挑选')
        # start date
        self.startdateVar = tkinter.StringVar()
        self.startdateVar.set('2018-01-08')
        self.startdateT = tkinter.Entry(self.wd, width=20, textvariable=self.startdateVar)
        self.startdateT.grid(column=1, row=0)
        self.lbsd = tkinter.Label(self.wd, text="Start Date: ")
        self.lbsd.grid(column=0, row=0)
        # end date
        self.enddateVar = tkinter.StringVar()
        self.enddateVar.set('2018-01-09')
        self.enddateT = tkinter.Entry(self.wd, width=20, textvariable=self.enddateVar)
        self.enddateT.grid(column=1, row=1)
        self.lbed = tkinter.Label(self.wd, text="End Date: ")
        self.lbed.grid(column=0, row=1)
        # work directory
        self.wdentryText = tkinter.StringVar()
        self.workdirT = tkinter.Entry(self.wd, width=20, textvariable=self.wdentryText)
        self.workdirT.grid(column=1, row=2)
        self.lbwd = tkinter.Label(self.wd, text="Work Directory: ")
        self.lbwd.grid(column=0, row=2)
        self.btnWorkDir = tkinter.Button(self.wd, text="...", command=self.getWorkDir)
        self.btnWorkDir.grid(column=2, row=2)
        # path/row definition
        self.prdefEntryText = tkinter.StringVar()
        self.prdefT = tkinter.Entry(self.wd, width=20, textvariable=self.prdefEntryText)
        self.prdefT.grid(column=1, row=3)
        self.lbprd = tkinter.Label(self.wd, text="Path/Row Definition: ")
        self.lbprd.grid(column=0, row=3)
        self.btnPrdef = tkinter.Button(self.wd, text="...", command=self.getprdef)
        self.btnPrdef.grid(column=2, row=3)
        # reference directory
        self.refEntryText = tkinter.StringVar()
        self.refT = tkinter.Entry(self.wd, width=20, textvariable=self.refEntryText)
        self.refT.grid(column=1, row=4)
        self.lbref = tkinter.Label(self.wd, text="Reference Dir: ")
        self.lbref.grid(column=0, row=4)
        self.btnRef = tkinter.Button(self.wd, text='...', command=self.getrefname)
        self.btnRef.grid(column=2, row=4)
        # index filename
        self.indexEntryText = tkinter.StringVar()
        self.indexT = tkinter.Entry(self.wd, width=20, textvariable=self.indexEntryText)
        self.indexT.grid(column=1, row=5)
        self.btnindex = tkinter.Button(self.wd, text='...', command=self.getindexFile)
        self.btnindex.grid(column=2, row=5)
        self.lbindex = tkinter.Label(self.wd, text="Index File: ")
        self.lbindex.grid(column=0, row=5)
        # multi thread
        self.multiVar = tkinter.IntVar()
        self.multiVar.set(2)
        self.mltspin = tkinter.Spinbox(self.wd, from_=1, to=30, width=5, textvariable=self.multiVar)
        self.mltspin.grid(column=1, row=6)
        self.lbmlt = tkinter.Label(self.wd, text="Num of Threads: ")
        self.lbmlt.grid(column=0, row=6)
        # run
        self.btn = tkinter.Button(self.wd, text="Run", command=self.RunMain)
        self.btn.grid(row=7)
        self.wd.mainloop()
    def getWorkDir(self):
        self.workdirPath = filedialog.askdirectory()
        self.wdentryText.set(self.workdirPath)
    def getindexFile(self):
        self.indexPath = filedialog.askopenfilename()
        self.indexEntryText.set(self.indexPath)
    def getprdef(self):
        self.prdefPath = filedialog.askopenfilename()
        self.prdefEntryText.set(self.prdefPath)
    def getrefname(self):
        self.refpath = filedialog.askdirectory()
        self.refEntryText.set(self.refpath)
    def clicked(self):
        messagebox.showinfo("Message title", "{}\n{}\n{}".format(self.refEntryText.get(),self.wdentryText.get(),self.indexEntryText.get()))
    def RunMain(self):
        df, _ = split_collection(self.indexEntryText.get())
        thumb_root = path.join(self.wdentryText.get(), '0.thumbnail')
        good = BestsceneWoker(self.refEntryText, self.prdefEntryText, self.startdateVar, self.enddateVar, thumb_root,
                              copydir=path.join(self.wdentryText.get(), '1.good'), df=df,
                              Global_monthlist=[4, 5, 6, 7, 8, 9, 10],
                              nprocess=self.multiVar.get())
